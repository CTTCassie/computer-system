...
if (isset($_POST['flag'])) {
    ...
    try {
        ...
        if ($_POST['flag'] == "insert") {
            $sql = "INSERT INTO `postgraduate`(`name`, `year`, `pigeonhole`, `award_name`, `award_time`, `rank`, `licenceauth`, `awark_class`, `time`) VALUES ('{$_POST['name']}','{$_POST['year']}','{$pigeonhole}','{$_POST['award_name']}','{$_POST['award_time']}','{$_POST['rank']}','{$_POST['licenceauth']}','{$_POST['awark_class']}','{$_POST['time']}')";
        } else {    //更新数据
...
if(isset($_POST['pipe'])){   //归档
    $pigeonhole = 1;
    try{
        $sql = "UPDATE `postgraduate` SET `pigeonhole`='{$pigeonhole}' WHERE `name`='{$_POST['name']}' and `time`='{$_POST['time']}' ";
        $st = $pdo->exec($sql);
        echo true;
    }
    catch(PDOException $ex){
...
if(isset($_POST['check'])){  //审核
    $pigeonhole = 1;
    try{
        $sql = "SELECT * FROM `postgraduate` WHERE `pigeonhole`='{$pigeonhole}' and `name`='{$_POST['name']}' and `time`='{$_POST['time']}' ";
        $st = $pdo->exec($sql);
        echo true;
    }
    catch(PDOException $ex){
