   ...
   <?php  
                ...
                $sql_query = "SELECT * FROM `graduate` WHERE `name`='{$name}'";
                if(!empty($year)){
                    $sql_query = "SELECT * FROM `graduate` WHERE `name`='{$name}' and `year`='{$year}'";
                }
                if(!empty($year) && !empty($date1) && !empty($date2)){
                    $sql_query = "SELECT * FROM `graduate` WHERE `name`='{$name}' and `year`='{$year}' and (`award_time` between '{$date1}' and '{$date2}')";
                }
                if(!empty($year) && !empty($date1) && !empty($date2) && $awark_class != 'empty'){
                    $sql_query = "SELECT * FROM `graduate` WHERE `name`='{$name}' and `year`='{$year}' and (`award_time` between '{$date1}' and '{$date2}') and `awark_class`='{$awark_class}'";
                }
                if(!empty($year) && !empty($date1) && !empty($date2) && $awark_class != 'empty' && !empty($licenceauth)){
                    $sql_query = "SELECT * FROM `graduate` WHERE `name`='{$name}' and `year`='{$year}' and (`award_time` between '{$date1}' and '{$date2}') and `awark_class`='{$awark_class}' and `licenceauth`='{$licenceauth}'";
                }
